            ...
            db.oauth_clients.save({
                ...
                if (!err) {
                    ...
                    res.end(JSON.stringify({
                        ...
                        client_id: clientId,
                        client_secret: clientSecret,
                        msg: "User saved. Feel free to get an access token from /oauth/token now"
                    }));
                }
            })
        } else {
            res.end(JSON.stringify({
                msg: "It seems that we're really lame. We tried giving you an already existing client id",
                status: 'failure'
            }))
        }
    })
    ...
    if (username && username.match('^[A-Za-z0-9-_\^]{5,30}$') && password && password.length > 2) {
        db.users.find({
            ...
            if (!users || !users.length) {
                ...
                res.end(JSON.stringify({
                    msg: "Try using another username",
                    status: 'failure'
                }));
            }
        })
        ...
        res.end(JSON.stringify({
            msg: "Username does not meet the required format or password/redirect uri are too short.",
            status: 'failure'
        }));
    }
};
...
exports.postLogin = function(req, res) {
    ...
    if (req.body.username && req.body.password) {
        ...
        res.end(JSON.stringify({
            msg: "No user credentials provided",
            'status': 'failure'
        }))
